...
RSpec.describe Reservation, type: :model do
  ...
  describe 'associations' do
    it 'belongs to a section' do
      reservation = Reservation.new(city: 'New York', date: Date.today, number_of_person: 2, section:, user:)
  ...
  describe 'validations' do
    ...
    it 'is not valid without a city' do
      reservation = Reservation.new(date: Date.today, number_of_person: 2, section:, user:)
      expect(reservation).to_not be_valid
      expect(reservation.errors[:city]).to include("can't be blank")
    end

    it 'is not valid without a date' do
      reservation = Reservation.new(city: 'New York', number_of_person: 2, section:, user:)
      expect(reservation).to_not be_valid
      expect(reservation.errors[:date]).to include("can't be blank")
    end

    it 'is not valid with a non-integer number of persons' do
      reservation = Reservation.new(city: 'New York', date: Date.today, number_of_person: 2.5, section:, user:)
      expect(reservation).to_not be_valid
